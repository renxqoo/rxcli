...
> **数据源 / 上游**:[vikiboss/60s](https://github.com/vikiboss/60s) 开源项目(MIT License © Viki)。默认实例 `https://60s.viki.moe`,全部接口在 `/v2` 前缀下。所有数据归上游原项目及原始数据源所有,本项目(rxopen)仅做 CLI 化封装,不持有/缓存任何数据。
>
> 本文是 CLI 化的接口契约,每条接口都封装为一个 `rxopen` 子命令。
...
把约 50 个接口按语义聚类成 11 个 namespace + 6 个顶层快捷命令。CLI 入口 `rxopen`。
...
- `rxopen daily` → `news today`(项目核心)
- `rxopen bing` → 必应每日壁纸(独立,不便归类)
- `rxopen toutiao` / `weibo` / `zhihu` → 热搜直达
- `rxopen hitokoto` → 一言直达
- `rxopen moyu` → 摸鱼日历直达
...
- **CLI**:`rxopen news today`(顶层快捷 `rxopen daily`)
- **参数**:
...
- **CLI**:`rxopen hot weibo`(顶层快捷 `rxopen weibo`)
- **参数**:无
- **响应 `data`**:
  ```ts
...
- **CLI**:`rxopen hot zhihu`(顶层快捷 `rxopen zhihu`)
- **参数**:无
- **响应 `data`**:
  ```ts
  [{
...
- **CLI**:`rxopen tech hackernews [--type best]`
- **参数**:
...
- **CLI**:`rxopen fun hitokoto`(顶层快捷 `rxopen hitokoto`)
- **参数**:
...
- **CLI**:`rxopen fun moyu`(顶层快捷 `rxopen moyu`)
- **参数**:
...
- **CLI**:`rxopen music rank-detail <id>`
- **参数**:
...
- **CLI**:`rxopen music lyric <query> [--clean false]`
- **参数**:
...
| 路由 | CLI | 说明 |
|---|---|---|
| `GET /v2/maoyan/all/movie` | `rxopen movie maoyan-all` | 全球电影票房总榜 |
| `GET /v2/maoyan/realtime/movie` | `rxopen movie maoyan-realtime --type movie` | 今日实时票房 |
| `GET /v2/maoyan/realtime/tv` | `rxopen movie maoyan-realtime --type tv` | 今日实时电视收视 |
| `GET /v2/maoyan/realtime/web` | `rxopen movie maoyan-realtime --type web` | 今日实时网播热度 |
...
| 路由 | CLI | 说明 |
|---|---|---|
| `/douban/weekly/movie` | `rxopen movie douban --cat movie` | 一周口碑电影榜 🎬 |
| `/douban/weekly/tv_chinese` | `rxopen movie douban --cat tv_chinese` | 国内剧集榜 📺 |
| `/douban/weekly/tv_global` | `rxopen movie douban --cat tv_global` | 全球剧集榜 🌍 |
| `/douban/weekly/show_chinese` | `rxopen movie douban --cat show_chinese` | 国内综艺榜 🎤 |
| `/douban/weekly/show_global` | `rxopen movie douban --cat show_global` | 全球综艺榜 🌍 |
...
- **CLI**:`rxopen life forecast [北京] [--days 7]`
- **参数**:
...
- **CLI**:`rxopen life fuel-price [--region 北京]`
- **参数**:
...
- **CLI**:`rxopen life exchange-rate [--currency CNY]`
- **参数**:
...
- **CLI**:`rxopen life lunar [--date <date>]`
- **参数**:
...
- **CLI**:`rxopen life today-in-history [--date <date>]`
- **参数**:
...
- **CLI**:`rxopen life olympics [--id <event-slug>]`
- **参数**:
...
- **CLI**:`rxopen life olympics-events`
- **参数**:无
...
- **CLI**:`rxopen tool qrcode <text> [--size 256] [--level M]`
- **参数**:
...
- **CLI**:`rxopen tool password [--length 16] [开关 flags]`
- **参数**(全可选):
...
- **CLI**:`rxopen tool password-check <password>`
...
- **CLI**:`rxopen tool color-palette [color]`
...
- **CLI**:`rxopen tool chemical [--id <id>]`
...
- **中文名**:有道翻译(调用有道网页翻译接口)
- **CLI**:`rxopen tool fanyi <text> [--from auto] [--to auto]`
- **参数**(支持 query 和 body):
...
- **CLI**:`rxopen kb js-question [--id <id>]`
- **参数**:`id`(int, 可选;省略随机)
...
- **CLI**:`rxopen health assess --height <cm> --weight <kg> --gender <male|female> --age <years>`
- **参数**(全 **required**):
...
- **数据源/baseUrl**:可配置(默认 `https://60s.viki.moe`),通过 `--base-url` 或环境变量 `RXOPEN_BASE_URL`。生产建议自部署。
