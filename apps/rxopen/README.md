...
Sync the 8 skills to your AI-tool discovery dirs (`~/.agents` always + any installed tool like `~/.claude`/`~/.cursor`/`~/.zcode`, auto-detected — for Claude Code / Cursor / Codex / ZCode / OpenClaw / Pi / Trae):
...
Covers 60+ categories of public data with 6 query skills and 2 workflow skills:
...
| `rxopen-morning` | workflow | Combined morning brief with weather, news, work calendar, almanac, and history |
| `rxopen-trending` | workflow | Cross-platform trending-topic overlap and differences |
...
Skill docs: 8 `skills/rxopen-*/SKILL.md` files. The 6 query skills receive generated command blocks from `rxopen skills gen <name>`; the 2 workflow skills contain orchestration instructions only. Build generates the shared installation guide into each skill's `references/install.md`. The API contract is in `docs/API.md`.

## Technical decisions

- **Naming**: npm package `@renxqoo/rxopen-cli` / bin command `rxopen` / 8 independently installable skills / **no credentialNamespace** (fully public data, no auth attached).
...
- **Skill layers**: `skillsScopes` generates in-scope command tables for the 6 query skills; `rxopen-morning` and `rxopen-trending` remain workflow-only and have no generated command table.
