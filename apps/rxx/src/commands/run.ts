 ...
 *
 * 拆成两步(S4):
 *   - assembleApp(manifest):manifest → defineCliApp App(纯装配,可复用/可测试)
 *   - runService(name, args):加载 manifest + assembleApp + app.run(编排)
 *
 * 核心:每次 rx run,按 argv 选服务 → 现场 build 一个 App → app.run(rest)。
 * defineCliApp 的 routed 数组是装配期不可变,所以每个动态服务现场 build 自己的 App。
 */

import { defineCliApp, type App } from "@renxqoo/agent-data-cli";
...
/**
 * 把 manifest 装配成一个 defineCliApp App(纯装配,不发请求)。
 * dir 在运行时解析(RXX_HOME 生效);auth 插件的异步装配由 defineCliApp 的 apply 完成。
 * 拆出来便于测试和未来扩展(dry-run / 缓存预热)。
 */
export async function assembleApp(manifest: Manifest): Promise<App> {
  const auth = buildAuthFromManifest(manifest);
