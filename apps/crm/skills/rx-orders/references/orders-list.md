...
| `--cursor` | string | 续拉游标,取上次响应的 `meta.pagination.next_token`         |
...
```bash
rxcli orders list --limit 5 --cursor <上次 meta.pagination.next_token>
```

CLI 会把服务端的 `hasMore` / `nextCursor` 转换成统一的 `meta.pagination.complete` / `next_token`。`complete:false` 时继续拉取，`complete:true` 时停止。

## 边界情况

- **无订单**:返回 `{ "orders": [] }`,不是错误
- **游标不存在**:服务端从当前用户的第一条订单重新开始；调用方应原样使用上次返回的 `next_token`,不要自行构造
