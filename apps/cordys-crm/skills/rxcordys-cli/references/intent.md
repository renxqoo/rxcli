...
"团队今天""公司情况""今天回款""合同审批追踪"等指令依赖**角色判断 + 数据范围切换**(SELF/DEPARTMENT/ALL)。未确认角色时按销售(自己)视角处理。

**按角色(经理/高管/财务/商务)组织的晨会/周会/月会命令时间表 → 见 [role.md](role.md) §工作流时间表**(本文件不再重复)。role.md 同时定义 searchType/deptIds 如何按角色取范围。
