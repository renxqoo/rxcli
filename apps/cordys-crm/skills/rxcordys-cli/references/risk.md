...
> 查询结果展示后,按本文件规则**主动扫描并预警**。

> ⚠️ **字段说明**:下文字段名经实测确认;不同部署的字段可能因配置不同而异,以 `rxcordys <ns> form` 或实际返回为准。已验证的关键字段:
...
- **给具体建议,不只报数字**:不说"有 3 条线索超期",要说"YYY 集团已 7 天未跟进,建议今日优先联系"
...
预警不是非黑即白,要按阈值分级(🟢正常 / ⚠️警戒 / 🚨严重)。下表是**经验默认阈值(基准参考,非硬性事实,应按业务基线调校)**,扫描时据此决定报 ⚠️ 还是 🚨。
...
> **followTime 判定要点**(实测):部分线索 `followTime` 为 `0` 或 `null`(从未跟进)。这类要判为"从未跟进"(🚨 高危),**不能**当成"超期 N 天"。计算天数前先判空:`followTime > 0` 才算超期天数,否则标记"从未跟进"。
