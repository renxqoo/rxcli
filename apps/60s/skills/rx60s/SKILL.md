...
> 每类命令的完整参数、枚举值、返回字段见对应 `references/<域>.md`。**构造精确调用或解析返回字段前,先读对应 reference**。

### 顶层快捷命令(高频直达)

```bash
rx60s 60s                       # 每天 60 秒读懂世界(项目核心)
rx60s bing                      # 必应每日壁纸
rx60s weibo / zhihu / toutiao   # 各平台热搜直达
rx60s hitokoto                  # 一言
rx60s moyu                      # 摸鱼办·打工人日历
```

### news —— 新闻资讯

```bash
rx60s news today                # 每天 60 秒(可 --date YYYY-MM-DD)
rx60s news ai                   # AI 资讯快报
rx60s news it [--limit 20]      # IT 之家资讯
rx60s news it-rank --type week  # IT 排行(day|week|month)
rx60s news rss                  # 近 7 天 RSS(解析为结构化)
```
...
rx60s hot zhihu                 # 知乎热搜
rx60s hot toutiao               # 头条热搜
rx60s hot douyin                # 抖音热搜
rx60s hot bili                  # B 站热搜
rx60s hot rednote               # 小红书热点
rx60s hot baidu-hot             # 百度热搜
rx60s hot baidu-teleplay        # 百度电视剧榜
rx60s hot baidu-tieba           # 百度贴吧
rx60s hot dongchedi             # 懂车帝热搜
rx60s hot quark                 # 夸克热点
```

### life —— 生活服务

```bash
rx60s life weather 北京         # 实时天气(位置参数:城市)
rx60s life forecast 上海 --days 5  # 天气预报
rx60s life fuel-price --region 广东  # 今日油价
rx60s life gold-price           # 贵金属金价
rx60s life exchange-rate --currency USD  # 汇率
rx60s life lunar                # 老黄历/万年历
rx60s life today-in-history     # 历史上的今天
rx60s life olympics             # 奥运奖牌榜
```

### tool —— 开发者工具

```bash
rx60s tool fanyi "hello" --to zh-CHS   # 有道翻译(位置参数)
rx60s tool hash <content>              # Hash/编码转换
rx60s tool qrcode <text>               # 二维码(返回 base64)
rx60s tool whois <domain>              # 域名 WHOIS
rx60s tool ip [--ip 8.8.8.8]           # IP 归属
rx60s tool password --length 20        # 密码生成
rx60s tool password-check <pwd>        # 密码强度检测
rx60s tool color "#FF5733"             # 颜色信息(注:用 --color)
rx60s tool chemical [--id 5]           # 化学物质
```

### fun —— 趣味文案

```bash
rx60s fun hitokoto              # 一言
rx60s fun duanzi                # 段子
rx60s fun kfc                   # 疯狂星期四文案
rx60s fun luck                  # 今日运势
rx60s fun fabing --name 主人    # 发病文学
```

### music / movie / tech / kb / health / beta

```bash
rx60s music rank                # 网易云榜单列表
rx60s music rank-detail 3778678 # 榜单详情
rx60s music lyric "稻香"         # 歌词搜索
rx60s movie douban --cat movie  # 豆瓣口碑榜
rx60s movie epic                # Epic 免费游戏
rx60s tech hackernews --type best  # Hacker News
rx60s kb baike 人工智能          # 百度百科
rx60s health assess --height 175 --weight 70 --gender male --age 30  # BMI
rx60s beta qq 123456           # QQ 信息(实验性)
```
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --date <string> | string | 否 | — | 指定日期 YYYY-MM-DD(默认今天,失败回退昨天/前天) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --id <number> | number | 否 | — | 指定索引(越界返回 not_found) |

**moyu**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --date <string> | string | 否 | — | 指定日期(默认今天) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --date <string> | string | 否 | — | 指定日期 YYYY-MM-DD(默认昨天) |
| --all | boolean | 否 | — | 返回全部日期的新闻(忽略 date) |

**news it**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --limit <number> | number | 否 | 20 | 返回条数上限(默认 20,上限 50) |

**news it-rank**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --type <string> | string | 否 | day | 榜单类型:day | week | month(默认 day) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --type <string> | string | 否 | top | 文章类型:top(热门) | best(精选)| new(⚠️上游 bug,等同 top) |
| --limit <number> | number | 否 | 10 | 返回数量(默认 10,上限 35) |
| --forceUpdate | boolean | 否 | — | 跳过缓存强制刷新(缓存 10 分钟) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --name <string> | string | 否 | — | 替换模板里的名字(默认'主人') |
| --id <number> | number | 否 | — | 指定模板索引(越界返回 not_found) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <id> | string | 是 | — | 榜单 ID(如 3778678 热歌榜;用 rank 查全部) |
| --size <number> | number | 否 | — | 返回曲目上限 |

**music lyric**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <query> | string | 是 | — | 歌曲/歌手关键词 |
| --clean | boolean | 否 | — | 过滤词/曲/编曲等元信息行(默认 true,设 false 保留) |
...
**movie maoyan-realtime**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --type <string> | string | 否 | movie | 类型:movie(实时票房) | tv(电视收视) | web(网播热度) |
| --date <string> | string | 否 | — | 日期 YYYYMMDD(默认今天) |

**movie douban**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --cat <string> | string | 否 | movie | 分类:movie(电影) | tv_chinese(国内剧) | tv_global(全球剧) | show_chinese(国内综艺) | show_global(全球综艺) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| [<query>] | string | 否 | — | 城市搜索词(默认北京,如 上海/广州天河) |
| --city <string> | string | 否 | — | 精确匹配城市名 |
| --province <string> | string | 否 | — | 精确匹配省份名 |

**life forecast**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| [<query>] | string | 否 | — | 城市搜索词(默认北京) |
| --days <number> | number | 否 | 7 | 预报天数(日预报上限 8,日出日落上限 15) |
| --city <string> | string | 否 | — | 精确匹配城市名 |
| --province <string> | string | 否 | — | 精确匹配省份名 |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --region <string> | string | 否 | — | 区域名(后缀匹配,如 北京/广东,默认北京) |
| --forceUpdate | boolean | 否 | — | 跳过缓存强制刷新(缓存 60 分钟) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --currency <string> | string | 否 | — | 基准货币代码 ISO 4217(如 USD,默认 CNY) |

**life lunar**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --date <string> | string | 否 | — | 日期(支持 10/13 位时间戳或日期字符串,默认今天) |

**life today-in-history**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --date <string> | string | 否 | — | 日期 ISO(默认今天) |

**life olympics**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --id <string> | string | 否 | — | 赛事 slug(省略取进行中的赛事) |

**life olympics-events**
(无参数)

**tool hash**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <content> | string | 是 | — | 待处理内容 |

**tool qrcode**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <text> | string | 是 | — | 要编码的文本 |
| --size <number> | number | 否 | — | 图片尺寸 px(默认 256) |
| --level <string> | string | 否 | — | 纠错级别 L/M/Q/H(默认 M) |

**tool og**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <url> | string | 是 | — | 目标 URL(无协议头自动补 https://) |

**tool whois**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <domain> | string | 是 | — | 域名(可含子域/协议头,自动提取根域) |

**tool ip**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --ip <string> | string | 否 | — | 指定 IP(省略则查调用者公网 IP) |

**tool password**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --length <number> | number | 否 | — | 密码长度(默认 16,范围 4-128) |
| --numbers | boolean | 否 | — | 包含数字(默认开,false/0 关) |
| --symbols | boolean | 否 | — | 包含特殊符号(默认关,true/1 开) |
| --lowercase | boolean | 否 | — | 包含小写字母(默认开) |
| --uppercase | boolean | 否 | — | 包含大写字母(默认开) |
| --excludeSimilar | boolean | 否 | — | 排除相似字符 il1Lo0O(默认开) |
| --excludeAmbiguous | boolean | 否 | — | 排除模糊字符 {}[]()/等(默认开) |

**tool password-check**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <password> | string | 是 | — | 待检测密码(≤128) |

**tool color**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --color <string> | string | 否 | — | HEX 颜色(如 #FF5733,省略随机) |

**tool color-palette**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --color <string> | string | 否 | — | 基础 HEX 颜色(省略随机) |

**tool chemical**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --id <string> | string | 否 | — | ChemSpider ID(省略随机) |

**tool fanyi**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <text> | string | 是 | — | 待翻译文本 |
| --from <string> | string | 否 | — | 源语言代码(默认 auto,见 fanyi-langs) |
| --to <string> | string | 否 | — | 目标语言代码(默认 auto) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --id <number> | number | 否 | — | 指定题目 ID(省略随机) |

**health assess**
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| --height <number> | number | 是 | — | 身高 cm(50-300) |
| --weight <number> | number | 是 | — | 体重 kg(10-300) |
| --gender <string> | string | 是 | — | 性别:male | female |
| --age <number> | number | 是 | — | 年龄(1-150) |
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| <qq> | string | 是 | — | QQ 号(5-11 位数字) |
| --size <number> | number | 否 | — | 头像尺寸:0 | 40 | 100 | 160 | 640(默认 0) |
<!-- AUTO-GEN:END -->
