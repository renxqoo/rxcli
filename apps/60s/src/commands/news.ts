 ...
 *
 * 子命令:
 *   - today             每天 60 秒读懂世界(项目核心)
 *   - ai                AI 资讯快报
 *   - it                IT 之家实时资讯
 *   - it-rank           IT 之家排行榜
 *   - rss               60s RSS 订阅(近 7 天,返回 XML,CLI 解析为结构化)
 *
 * 数据源:60s-api(默认 https://60s.viki.moe),全部 /v2 前缀。
 */
...
export const newsCommands = defineCommands({
  // ── 每天 60 秒读懂世界 ──────────────────────────────────────────
  today: defineCommand<{ date?: string; forceUpdate?: boolean }>({
    name: "today",
    description: "每天 60 秒读懂世界(每日新闻摘要 + 微语)",
    args: {
      date: { type: "string", desc: "指定日期 YYYY-MM-DD(默认今天,失败回退昨天/前天)" },
      forceUpdate: { type: "boolean", desc: "跳过缓存强制刷新" },
    },
    humanFormat(data) {
    ...
    async run({ date, forceUpdate }, ctx) {
  ...
  // ── AI 资讯快报 ─────────────────────────────────────────────────
  ai: defineCommand<{ date?: string; all?: boolean }>({
    name: "ai",
    description: "AI 资讯快报(每日 AI 行业新闻)",
    args: {
      date: { type: "string", desc: "指定日期 YYYY-MM-DD(默认昨天)" },
      all: { type: "boolean", desc: "返回全部日期的新闻(忽略 date)" },
    },
    humanFormat(data) {
  ...
  // ── IT 之家实时资讯 ─────────────────────────────────────────────
  it: defineCommand<{ limit?: number }>({
    name: "it",
    description: "IT 之家实时资讯",
    args: { limit: { type: "number", default: 20, desc: "返回条数上限(默认 20,上限 50)" } },
    humanFormat(data) {
  ...
  // ── IT 之家排行榜 ───────────────────────────────────────────────
  "it-rank": defineCommand<{ type?: string }>({
    name: "it-rank",
    ...
    args: {
      type: {
        type: "string",
        default: "day",
        desc: "榜单类型:day | week | month(默认 day)",
      },
    },
    humanFormat(data) {
  ...
  // ── 60s RSS 订阅(返回 XML,CLI 解析为结构化) ────────────────────
  rss: defineCommand({
    name: "rss",
    description: "60s RSS 订阅(近 7 天新闻)",
    async run(_args, ctx) {
