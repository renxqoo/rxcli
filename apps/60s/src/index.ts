...
const app = defineCli<Rx60sState>({
  name: "rx60s",
  binName: "rx60s",
  description:
...
if (isMainEntry()) {
  ...
  // install 向导(无鉴权场景主要用于 skills 同步到 ~/.agents/skills/)
  if (argv[0] === "install") {
    void (async () => {
      const { runInstallWizard } = await import("@renxqoo/agent-data-cli");
      const code = await runInstallWizard({ skillsSource: process.env.RX60S_SKILLS_SOURCE });
      process.exit(code);
    })();
  } else {
    app.run(argv).catch(() => {
