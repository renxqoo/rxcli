# 错误完整目录 + errorOnStatus 推荐配置（中文）

> 在这里查询 9 个错误 Category、标准 subtype、退出码和 `errorOnStatus` 配置。

## 导航

1. Category、subtype 与退出码
2. `errorOnStatus` 配置
3. 类型化错误示例
4. 注册、包装、脱敏与 `BareError`
...
throw new errs.PermissionError({
  ...
  hint: "联系管理员授权 orders:write；更新客户端 scope 后重新登录",
