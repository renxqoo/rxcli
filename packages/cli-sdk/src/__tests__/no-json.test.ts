...
describe('--no-json: 默认 JSON 不变', () => {
  it('不传 flag → JSON 信封', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        hello: defineCommand({ name: 'hello', description: 'x', async run() { return { data: { a: 1 } } } }),
      },
    })
    await app.run(['hello'])
...
describe('--no-json: 人类可读文本(通用兜底)', () => {
  ...
  it('--no-json 对象数组 → 自动表格(取对象 keys 当列)', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        list: defineCommand({ name: 'list', description: 'x', async run() { return { data: [{ id: 'o1' }, { id: 'o2' }] } } }),
      },
    })
  ...
  it('--no-json {key:array} 单 key 包装 → 钻进去自动表格', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        list: defineCommand({ name: 'list', description: 'x', async run() { return { data: { orders: [{ id: 'o1', total: 199 }] } } } }),
      },
    })
    await app.run(['list', '--no-json'])
  ...
  it('--no-json {key:array, total:n} 多 key 包装 → 钻进首个数组(忽略伴随字段)', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        list: defineCommand({ name: 'list', description: 'x', async run() { return { data: { products: [{ id: 'p1', price: 39 }], total: 1 } } } }),
      },
    })
    await app.run(['list', '--no-json'])
  ...
  it('--no-json scalar 数组 → 序号列表', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        list: defineCommand({ name: 'list', description: 'x', async run() { return { data: ['a', 'b', 'c'] } } }),
      },
    })
    await app.run(['list', '--no-json'])
    // scalar 项 → 序号列表
  ...
  it('--no-json meta count → 附摘要行', async () => {
    const app = defineCli({
      ...
      commands: {
        list: defineCommand({
          name: 'list', description: 'x',
          async run() { return { data: [{ id: 'o1' }], meta: { count: 1, pagination: { complete: true } } } },
        }),
      },
    })
    await app.run(['list', '--no-json'])
  ...
  it('--no-json null data → （无数据）', async () => {
    const app = defineCli({
      ...
      description: 'd',
      commands: {
        void: defineCommand({ name: 'void', description: 'x', async run() { return { data: null } } }),
      },
    })
...
describe('printTable: 表格渲染工具', () => {
  ...
  it('基本表格:标题行 + 分隔行 + 数据行,列对齐', () => {
    const out = printTable(
      [{ id: 'o1', total: 199 }, { id: 'o100', total: 5 }],
      [
        { header: 'ID', value: (r: { id: string; total: number }) => r.id },
        { header: '总额', value: (r: { id: string; total: number }) => `¥${r.total}`, align: 'right' },
      ],
    )
  ...
  it('value 返回 null/undefined → 空字符串(不崩)', () => {
    const out = printTable(
      [{ a: null }, { a: undefined }, { a: 'x' }] as Array<{ a: unknown }>,
...
describe('--no-json: 错误也文本化(stderr)', () => {
  it('--no-json 命令抛错 → prettyError 到 stderr', async () => {
    const app = defineCli({
      ...
      commands: {
        fail: defineCommand({
          name: 'fail', description: 'x',
          async run() { throw new errs.APIError({ subtype: 'not_found', code: 404, message: '订单不存在', hint: '检查 ID' }) },
        }),
      },
    })
  ...
  it('默认模式错误 → JSON 错误信封到 stderr(--no-json 不传)', async () => {
    const app = defineCli({
      ...
      commands: {
        fail: defineCommand({
          name: 'fail', description: 'x',
          async run() { throw new errs.NotFoundError({ subtype: 'not_found', message: '订单不存在' }) },
        }),
      },
    })
    await app.run(['fail'])
    ...
    const app = defineCli({
      ...
      commands: {
        hello: defineCommand({
          ...
          async run(args) { receivedArgs = args; return { data: { ok: true } } },
        }),
      },
    })
...
function mkDataApp(
  ...
  return defineCli({
    name: 'demo', description: 'd', defaultFormat,
    commands: {
      hello: defineCommand({ name: 'hello', description: 'x', async run() { return { data: { a: 1 } } } }),
    },
  })
...
function isJsonOut(buf: string): boolean {
  try { const e = JSON.parse(buf); return e.ok === true && 'data' in e } catch { return false }
}
...
describe('defaultFormat: auto(TTY→文本/非 TTY→JSON,默认值)', () => {
  ...
  it('不传 defaultFormat(省略)= auto 行为', async () => {
    ...
    const app = defineCli({
      ...
      commands: { hello: defineCommand({ name: 'hello', description: 'x', async run() { return { data: { a: 1 } } } }) },
    })
    await app.run(['hello'])
    expect(isTextOut(stdoutBuf)).toBe(true)
  })
