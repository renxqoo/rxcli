...
const listCommand = defineCommand({
  ...
  async run(args, ctx) {
    const res = await ctx.get<{ items: Array<{ id: string; total: number }>; hasMore: boolean }>('/orders', { limit: args.limit })
    return {
      data: res.data.items,
      meta: {
...
describe('pipeline: 成功路径(runCommand + 信封)', () => {
  it('list 返回数组 data + 分页 meta,stdout 是信封', async () => {
    const ctx = createTestCtx({
      request: async (opts) => ({
        status: 200,
        data: { items: [{ id: 'o1', total: 100 }, { id: 'o2', total: 200 }], hasMore: false },
        headers: {},
      }),
    })
    ...
    expect(env.data).toEqual([{ id: 'o1', total: 100 }, { id: 'o2', total: 200 }])
  ...
  it('data 业务字段 camelCase 不被转 snake_case', async () => {
    const ctx = createTestCtx({
      request: async () => ({ status: 200, data: { items: [{ userId: 'u1', orderCount: 3 }] }, headers: {} }),
    })
...
describe('pipeline: onError 插件链(错误归一化)', () => {
  ...
  it('onError 插件吞掉错误(return undefined)→ 命令变成功 + exit 0', async () => {
    ...
    const ctx = createTestCtx({ request: async () => ({ status: 404, data: {}, headers: {} }) })
    const code = await runCommand({ spec: getCommand, args: { id: 'x' }, ctx, plugins: [swallower] })
    expect(code).toBe(0)
    const env = JSON.parse(stdoutBuf)
...
describe('精确豁免:plugin 自己的命令跳自己的 beforeCommand(route 传入时)', () => {
  it('plugin A 贡献 cmd → 跑 cmd 时 A 的 beforeCommand 不跑(route 命中 _ownedRoutes)', async () => {
    ...
    const ctx = createTestCtx()
    const code = await runCommand({ spec: ownedCmd, args: {}, ctx, plugins: [pluginA], route: ['login'] })
    expect(code).toBe(0)
  ...
  it('plugin B(非 owner)的 beforeCommand 照跑(不误伤别的 plugin)', async () => {
    ...
    const ctx = createTestCtx()
    await runCommand({ spec: ownedCmd, args: {}, ctx, plugins: [pluginA, pluginB], route: ['login'] })
  ...
  it('namespace 路径的 ownedRoute 精确豁免(["auth","login"])', async () => {
    ...
    const ctx = createTestCtx()
    await runCommand({ spec: loginCmd, args: {}, ctx, plugins: [pluginA], route: ['auth', 'login'] })
