...
describe('401 singleflight: 并发复用同一次 refresh + 落盘', () => {
  it('并发 401 复用同一个 refresh Promise', async () => {
    ...
    const refreshSpy = vi.fn(async () => {
      refreshCalls++
      return { access_token: 'new_tok', refresh_token: 'new_rt', expires_in: 3600, scope: 'orders:read' }
    })

    const store = memoryStore({
      credentials: {
        orders: {
          token: 'old_tok', refreshToken: 'old_rt', expiresAt: 1, scopes: ['orders:read'],
          user: { userId: 'u1' }, storedAt: 1, authMethod: 'oauth',
        },
      },
    })
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url: URL | RequestInfo, init?: RequestInit) => {
      const u = String(url)
      const body = init?.body as string
      if (u.endsWith('/token') && body?.includes('refresh_token')) {
        return new Response(JSON.stringify(await refreshSpy()), { status: 200, headers: { 'content-type': 'application/json' } })
      }
  ...
  it('refresh 失败(refreshToken 失效)→ 返回 null', async () => {
    const store = memoryStore({
      credentials: {
        orders: { token: 'old', refreshToken: 'bad', expiresAt: 1, scopes: [], storedAt: 1, authMethod: 'oauth' },
      },
    })
    const cfg = { baseUrl: 'http://mock', clientId: 'c', clientSecret: 's' }
    const on401 = createOn401Hook({ cfg, store, namespace: 'orders' })
