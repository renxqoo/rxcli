...
import { toCliError, exitCodeOf, errs } from './errs/index.js'
...
export function defineCommand<Args = any, Result = unknown>(
  ...
  if (typeof spec.run !== 'function') throw new Error(`defineCommand(${spec.name}): run 必填且为函数`)
  return spec
}
...
export function defineCommands(group: CommandGroup): CommandGroup {
  for (const [key, cmd] of Object.entries(group)) {
    ...
    if (typeof cmd.run !== 'function') throw new Error(`defineCommands(${key}.${cmd.name}): run 必填且为函数`)
  }
  return group
}
      ...
      try {
        ...
        if (!matched) {
          ...
          if (argv.includes('-h') || argv.includes('--help') || argv.length === 0) {
            process.stdout.write(renderHelp(binName, routed) + '\n')
            process.exitCode = 0
          } else {
 ...
 */
function matchRoute(argv: string[], routed: Array<{ route: string[]; spec: CommandSpec }>): MatchResult {
 ...
 */
function parseFlags(tokens: string[], argsSpec: ArgsSpec | undefined): { options: Record<string, unknown>; positionals: string[] } {
...
function renderHelp(binName: string, routed: Array<{ route: string[]; spec: CommandSpec }>): string {
  const lines: string[] = []
...
/** 渲染单个命令的帮助(子命令 -h 时用)。 */
function renderCommandHelp(binName: string, matched: { route: string[]; spec: CommandSpec }): string {
  const sig = signatureOfArgs(matched.spec.args)
  const pos = sig.positionals.join(' ')
  const usage = `${binName} ${matched.route.join(' ')}${pos ? ' ' + pos : ''} ${sig.options.join(' ')}`.trim()
  const lines: string[] = []
...
async function executeOne<State>(opts: ExecuteOneOptions<State>): Promise<number> {
  ...
  const authPlugin = opts.plugins.find(
    (p): p is Plugin<State> & { _transportConfig?: { on401?: () => Promise<string | null> } } =>
      '_transportConfig' in p && (p as { _transportConfig?: unknown })._transportConfig !== undefined,
  )
