...
function handleCancel<T>(value: T, msg: Messages): T {
  if (p.isCancel(value)) {
...
function runSilentAsync(cmd: string, args: string[], opts: ExecFileSyncOptions = {}): Promise<Buffer> {
...
async function stepSelectLang(): Promise<'zh' | 'en'> {
...
async function stepInstallGlobally(msg: Messages, pkgName: string): Promise<void> {
  const installedVer = getGloballyInstalledVersion(pkgName)
  const latestVer = getLatestVersion(pkgName)
  const needsUpgrade =
    !!installedVer && !!latestVer && installedVer !== 'unknown' && semverLessThan(installedVer, latestVer)
 ...
 */
async function stepInstallSkills(msg: Messages, opts: { skillsSource?: string; binName: string }): Promise<void> {
  const s = p.spinner()
  ...
  try {
    ...
    throw new Error(`${opts.binName} not found and ${opts.skillsSource ? 'npx skills add failed' : 'no skillsSource'}`)
  } catch {
...
async function stepRegister(msg: Messages, binName: string): Promise<void> {
...
async function stepAuthLogin(msg: Messages, binName: string): Promise<void> {
  const rxcli = whichBin(binName)
  if (!rxcli) {
 ...
 * ```
 */
export async function runInstallWizard(opts: InstallWizardOptions = {}): Promise<void> {
  ...
  if (isInteractive) {
    p.intro(msg.setup)
    if (pkgName) await stepInstallGlobally(msg, pkgName)
    await stepInstallSkills(msg, { skillsSource: opts.skillsSource, binName })
    await stepRegister(msg, binName)
