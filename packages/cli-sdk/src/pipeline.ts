...
export async function runCommand<State>(opts: RunCommandOptions<State>): Promise<number> {
  ...
  try {
    ...
    if (meta && meta._rawOutput) {
      process.stdout.write(typeof transformed === 'string' ? transformed : String(transformed ?? ''))
      return 0
    }
    ...
    if (opts.humanReadable) {
      ...
      process.stdout.write(serializeSuccess(transformed, cleanMeta, { identity: resolveIdentity() }))
    }
    return 0
  } catch (err) {
