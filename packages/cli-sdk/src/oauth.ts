 ...
 */
export async function deviceAuthorization(cfg: OAuthClientConfig, scope?: string): Promise<DeviceAuthInfo> {
  ...
  if (!res.ok) throw new APIError({ subtype: 'server_error', code: res.status, message: 'device_authorization failed', cause: body })
 ...
 */
export async function pollDeviceToken(cfg: OAuthClientConfig, deviceCode: string): Promise<PollResult> {
...
export async function refreshAccessToken(cfg: OAuthClientConfig, refreshToken: string): Promise<TokenInfo> {
  const res = await fetch(`${cfg.baseUrl}/token`, {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body: new URLSearchParams({
...
export async function getUserInfo(cfg: OAuthClientConfig, accessToken: string): Promise<UserInfo> {
  ...
  const body = await safeJson(res)
  if (!res.ok) throw new APIError({ subtype: 'server_error', code: res.status, message: 'user_info failed', cause: body })
...
export function createOn401Hook(opts: {
  ...
  return async function on401(): Promise<string | null> {
    // 读当前凭证拿 refreshToken
    const creds = (await opts.store.loadCredentials(opts.namespace)) as Partial<StoredOAuthCredentials> | null
    ...
    if (!p) {
      p = refreshAccessToken(opts.cfg, refreshToken).finally(() => refreshInflight.delete(refreshToken))
    ...
    try {
      ...
      const updated: StoredOAuthCredentials = {
        ...
        scopes: newToken.scope ? newToken.scope.split(/\s+/).filter(Boolean) : (creds?.scopes ?? []),
        storedAt: Date.now(),
        authMethod: 'oauth',
        ...(creds?.user ? { user: creds.user } : {}),
      }
      await opts.store.saveCredentials(opts.namespace, updated as unknown as Record<string, unknown>)
